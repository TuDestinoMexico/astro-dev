...
export default function PromoLocker({ children }) {
    ...
    // NUEVA FECHA OBJETIVO: Jueves 11 de Junio, Partido Inaugural
    ...
    useEffect(() => {
        ...
        const calculateTime = () => {
            ...
            if (diff <= 0) {
                // Si ya inició el partido, las ofertas se desbloquean solas
                setIsRevealed(true);
            } else {
                setIsRevealed(false);
                setTimeLeft({
    ...
    return (
        <div className="relative w-full rounded-2xl overflow-hidden">
            ...
            {/* CONTENIDO BLOQUEADO (Tus banners de hoteles o tours ocultos por el blur) */}
            ...
            {/* OVERLAY DE BLOQUEO (Estilo Estadio/Mundialista) */}
            {!isRevealed && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-4">

                    {/* Tarjeta de Cristal Contenida */}
                    <div className="bg-white/95 backdrop-blur-md p-6 md:p-8 rounded-[2rem] shadow-2xl flex flex-col items-center max-w-md w-full border border-white/50 transform transition-transform hover:scale-105">
                        ...
                        {/* Ícono de Candado en Verde Esmeralda */}
                        <div className="w-16 h-16 bg-gradient-to-br from-emerald-600 to-green-700 rounded-full flex items-center justify-center mb-4 shadow-lg shadow-emerald-600/30">
                            <Lock className="text-white w-8 h-8" />
                        </div>

                        <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight text-center">
                            Tarifas Mundialistas
                        </h3>
                        <p className="text-slate-500 text-sm font-medium mb-6 text-center leading-snug">
                            Las promociones exclusivas del silbatazo inicial <span className="font-bold text-emerald-600">México vs Sudáfrica</span> se revelarán en:
                        </p>

                        {/* Cronómetro de Expectativa */}
                        <div className="flex items-center justify-center gap-2 md:gap-3 w-full">
                            ...
                            {/* Caja de Segundos destacada con el color de la Selección */}
                            <div className="flex flex-col items-center w-14">
                                <div className="w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-emerald-600 to-green-700 text-white font-black text-xl md:text-2xl flex items-center justify-center rounded-xl shadow-md border border-green-500/20">
                                    {timeLeft.seg}
                                </div>
                                <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1.5">Seg</span>
                            </div>
                        </div>
