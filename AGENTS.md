...
- **Auth:** Firebase Auth (email/contraseña, solo client-side)
...
15. **ClientPagos.jsx** usa los CTs vinculados en `users/{uid}/reservas` y los GBs en `users/{uid}/grupos` (Firestore) y consume `/api/crm-pagos` (proxy a `{API_CRM_URL}/api/pago/{ct}`) o `/api/crm-grupo-pagos` (proxy a `{API_CRM_URL}/api/gb/{gb}/pagos`) según el chip activo. No guarda datos en Firestore.
...
- Preferir `client:visible` o `client:idle` sobre `client:load` para islas React. Usar `client:only="react"` solo cuando el componente no debe renderizarse en SSR.
...
8. No ejecutar `npm run build` a menos que se solicite.
