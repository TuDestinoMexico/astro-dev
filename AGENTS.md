...
14. **ClientPagos.jsx** usa los CTs vinculados en `users/{uid}/reservas` y los GBs en `users/{uid}/grupos` (Firestore) y consume `/api/crm-pagos` (proxy a `{API_CRM_URL}/api/pago/{ct}`) o `/api/crm-grupo-pagos` (proxy a `{API_CRM_URL}/api/gb/{gb}/pagos`) según el chip activo. También lee el historial Openpay en `users/{uid}/pagos`; los cargos se crean server-side y no se escriben desde el cliente.
